...
namespace cdoge
{
    class Interpreter : Expr.Visitor<object>, Stmt.Visitor<object> // <Void> in jDoge
    {
        ...
        public object visitClassStmt(Stmt.Class stmt)
        {
            ...
            Dictionary<string, DogeFunction> methods = new Dictionary<string, DogeFunction>();
            foreach (Stmt.Function method in stmt.methods)
            {
                DogeFunction function = new DogeFunction(method, environment, method.name.lexeme.Equals("init"));
                methods.Add(method.name.lexeme, function);
            }

            DogeClass klass = new DogeClass(stmt.name.lexeme, (DogeClass)superclass, methods);
        ...
        {
            DogeFunction function = new DogeFunction(stmt, environment, false);
        ...
        public object visitCallExpr(Expr.Call expr)
        {
            ...
            if (!(callee is DogeCallable))
            {
                throw new RuntimeError(expr.paren, "Can only call functions and classes.");
            }

            DogeCallable function = (DogeCallable)callee;
        ...
        public object visitGetExpr(Expr.Get expr)
        {
            ...
            if (object_ is DogeInstance)
            {
                return ((DogeInstance)object_).get(expr.name);
            }
        ...
        public object visitSetExpr(Expr.Set expr)
        {
            ...
            object value = evaluate(expr.value);
            ((DogeInstance)object_).set(expr.name, value);
            return value;
        }
        ...
        public object visitSuperExpr(Expr.Super expr)
        {
            ...
            DogeClass superclass = (DogeClass)environment.getAt(distance, "super");

            // "this" is always one level nearer than "super"'s environment
            DogeInstance object_ = (DogeInstance)environment.getAt(distance - 1, "this");

            DogeFunction method = superclass.findMethod(expr.method.lexeme);
