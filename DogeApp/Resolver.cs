...
namespace cdoge
{
    class Resolver : Expr.Visitor<object>, Stmt.Visitor<object>
    {
        ...
        public object visitClassStmt(Stmt.Class stmt)
        {
            ...
            {
                Doge.error(stmt.superclass.name, "A class cannot inherit from itself.");
            }
        ...
        public object visitReturnStmt(Stmt.Return stmt)
        {
            ...
            {
                Doge.error(stmt.keyword, "Cannot return from top-level code.");
            }
            ...
            if (stmt.value != null)
            {
                ...
                {
                    Doge.error(stmt.keyword, "Cannot return a value from an initializer.");
                }
        ...
        public object visitSuperExpr(Expr.Super expr)
        {
            ...
            {
                Doge.error(expr.keyword, "Cannot use 'super' outside of a class.");
            }
            else if (currentClass != ClassType.SUBCLASS)
            {
                Doge.error(expr.keyword, "Cannot use 'super' in a class with no superclass.");
            }
        ...
        public object visitThisExpr(Expr.This expr)
        {
            if (currentClass == ClassType.NONE)
            {
                Doge.error(expr.keyword, "Cannot use 'this' outside of a class.");
                return null;
            }
        ...
        public object visitVariableExpr(Expr.Variable expr)
        {
            ...
            if (scopes.Count > 0 && scopes[scopes.Count - 1].ContainsKey(expr.name.lexeme))
            {
                if (scopes[scopes.Count - 1][expr.name.lexeme] == false) // declared but not yet defined
                    Doge.error(expr.name, "Cannot read local variable in its own initializer.");
            }
        ...
        private void declare(Token name)
        {
            ...
            {
                Doge.error(name, "Variable with this name already declared in this scope.");
            }
            else
            {
